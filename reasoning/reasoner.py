...
def _normalise_answer_key(text: str) -> str:
    tokens = re.findall(r"[a-z0-9]{3,}", (text or "").lower())
    return " ".join(tokens[:24])


def _source_label(source: dict) -> str:
    src = str(source.get("source", "")).strip() or str(source.get("type", "evidence"))
    return src


def _compose_attempt_answer(
    query: str, evidence: dict, verification: dict, confidence: float
) -> str:
    sources = sorted(
    ...
    lead_source = _source_label(lead)
    text = f"Best supported answer from {lead_source}: {lead_text}"

    if verification.get("issues") or verification.get("gaps") or confidence < 0.55:
        text = f"Limited-confidence answer: {text}"
    return text.strip()
...
_REASONING_TYPES = {
    "factual": re.compile(
        r"\b(what is|who is|when did|where is|define|meaning of)\b", re.IGNORECASE
    ),
    "comparative": re.compile(
        r"\b(compare|difference|vs\.?|versus|better|worse|pros and cons)\b",
        re.IGNORECASE,
    ),
    "causal": re.compile(
        r"\b(why|cause|reason|because|leads to|results in|due to)\b", re.IGNORECASE
    ),
    "procedural": re.compile(
        r"\b(how to|steps to|how do I|procedure|process|implement|build)\b",
        re.IGNORECASE,
    ),
    "evaluative": re.compile(
        r"\b(should I|is it|evaluate|assess|worth|recommend|best)\b", re.IGNORECASE
    ),
    "hypothetical": re.compile(
        r"\b(what if|suppose|imagine|if .* then|scenario|would)\b", re.IGNORECASE
    ),
    "summarization": re.compile(
        r"\b(summarize|summarise|overview|tldr|brief|main points|key points)\b",
        re.IGNORECASE,
    ),
    "creative": re.compile(
        r"\b(write|draft|generate|create|design|brainstorm|suggest)\b", re.IGNORECASE
    ),
}
...
def decompose(query: str, reasoning_type: str | None = None) -> list[str]:
...
def aggregate_evidence(
    ...
    for k in kg_facts[:5]:
        triple = f"{k.get('subject', '')} {k.get('relation', '')} {k.get('object', '')}"
        sources.append(
            {
                "type": "kg",
                "text": triple,
...
def verify_evidence(
    query: str, evidence: dict, reasoning_type: str = "factual"
) -> dict:
...
def estimate_confidence(
    evidence: dict, verification: dict, cognition: dict | None = None
) -> float:
    ...
    conf = 0.40 * ev_conf + 0.30 * coverage + 0.20 * verified + 0.10 * cog_conf
...
def synthesise(
    ...
    if confidence < 0.4:
        hedging = (
            "Note: limited evidence available — treat this as a best-effort answer."
        )
    elif confidence < 0.6:
    ...
    plan_parts.extend(
        f"Note: {adj}" for adj in verification.get("adjustments", [])
    )
    if hedging:
    ...
    trace = [
        ...
        {
            "step": "evidence",
            "result": f"{evidence.get('total', 0)} items from "
        ...
        },
        {
            ...
            else f"ISSUES: {verification.get('issues', [])}",
        },
...
class Reasoner:
    ...
    def assess(
        ...
        synthesis = synthesise(
            query, rtype, subgoals, evidence, verification, confidence
        )
    ...
    def assess_contract(
        ...
        """Stable attempt-level contract used by self-consistency."""
        reasoning = self.assess(
            query=query,
            memory=memory,
            chunks=chunks,
            kg_facts=kg_facts,
        ...
        verification = reasoning.get("verification", {})
        confidence = float(reasoning.get("confidence", 0.0))
        answer = _compose_attempt_answer(query, evidence, verification, confidence)
        citations = tuple(
        ...
        tool_usage = ()
        if tool and tool.get("tool"):
            tool_usage = (str(tool.get("tool")),)
        warnings = tuple(
            [str(item) for item in verification.get("issues", [])]
            + [str(item) for item in verification.get("gaps", [])]
        )
        evidence_strength = min(
            1.0,
            max(
                0.0,
                (float(evidence.get("confidence", 0.0)) * 0.6)
                + (min(float(evidence.get("total", 0)), 4.0) / 4.0 * 0.2)
                + (float(verification.get("coverage", 0.0)) * 0.2),
            ),
        )
        trace = tuple(
            f"{step.get('step', '')}: {step.get('result', '')}"
            for step in reasoning.get("trace", [])
        ...
        return {
            ...
            "answer_similarity_key": _normalise_answer_key(answer),
            "reasoning": reasoning,
        }
