...
8A. [Cognitive Runtime Additions (v1.0.0)](#8a-cognitive-runtime-additions-v100)
...
## 8A. Cognitive Runtime Additions (v1.0.0)

Everything in this section was built and independently verified after the rest of this guide was written. If you read Sections 1–8 and 14 first, treat this section as authoritative wherever it conflicts with them — the runtime request lifecycle now includes these stages in addition to what Section 13 describes.

Repository history for this work, in order: `de52e3e` (goal-aware planning) → `436f680` (ruff config cleanup) → `a07035b` (learning v1) → `0d28e56` (replay v1) → `1af9086` (adaptation v1, tagged `v1.0.0`). All six commits pass `pytest`, `ruff check .`, and `mypy --config-file mypy.ini` independently, not just as a combined tree.

### Self-Consistency (`self_consistency/`)

Runs multiple reasoning attempts over evidence-perturbed variants of the same query (baseline / reordered / trimmed / rotated evidence) and compares them at the **claim level**, not just by reasoning-trace shape — two attempts with near-identical reasoning steps but different final numbers are correctly flagged as disagreement. `self_consistency/claim_normalizer.py` collapses paraphrases of the same claim (`"top_k is 8"` ≡ `"the default retrieval top_k is 8"`) while keeping genuine numeric or action conflicts distinct. Output is one of four labels: `strong_agreement`, `superficial_agreement`, `false_agreement`, `clear_disagreement`.

```bash
python scripts/run_self_consistency_v3_validation.py
```

### Reflection (`reflection/`)

A single-shot, deterministic check run after generation: does the draft answer contradict the evidence, ignore evidence the query clearly asks about, or assert something no evidence supports? If a problem is found, the reasoner is asked once for a re-derived answer — never looped. Result is attached to `EngineResult.reflection` as `{"revised": bool, "reason": str}`.

### Goals (`goal/`) and Goal-Aware Planning (`planning/`)

Persistent, SQLite-backed goals (`active` / `paused` / `completed`) with real schema constraints — non-empty titles, valid status values, foreign-key-checked parent links, unique sibling titles. Managed via short-circuit `/goal` commands (`create`, `pause`, `resume`, `complete`, `split`) that bypass the reasoning path entirely.
...
/goal resume <id>
/goal complete <id>
/goal split <id>: <child one> | <child two>
/goal list
```

On top of that, `planning/` computes a deterministic focus for each turn — active before paused before completed, higher priority first, cycle-safe (a goal that is its own ancestor, directly or indirectly, is detected and blocked rather than causing infinite recursion). Planning output is attached to `result.trace["planning"]`; it never mutates goal state.

### Memory Admission (`memory_admission/`)

Every memory write is gated before it happens — not filtered after. A five-stage waterfall rejects empty/low-value text and near-duplicates, but explicitly admits corrections (`"actually, it's X, not Y"`) and reinforcement of important facts even when they'd otherwise look like duplicates. Importance is a weighted blend of caller hint, memory kind, lexical markers, and length; near-duplicates are rejected within a configurable recency window and re-admitted afterward only if they're still important.

### Learning (`learning`, wired into `engine.py`)

Uses the *existing* Memory Admission gate — no separate write path. Fires on exactly two triggers: a Reflection revision, or a Self-Consistency result of `strong_agreement` above a confidence threshold (default 0.70, strict inequality). Both triggers are independently gated by admission, so a rejecting policy suppresses both even if they fire in the same turn.

### Replay (`replay`, wired into `engine.py`)

Accumulates a structured, read-only trace of what happened during a request — planning decisions, reflection outcome, learning writes, memory admission events — without altering behavior or persisting anything on its own. This is the data Adaptation reads.

### Adaptation v1 (`_build_adaptation_trace` in `engine.py`)

Deliberately conservative first step: a pure function that classifies a request's replay trace (`stable`, `reflection_revision`, `learning_signal`, `admission_rejection`, `warning`, `error`) for observability. It does not change retrieval, prompts, thresholds, or model weights — it labels what happened, it does not yet act on the label. Verified with an explicit no-mutation test (`test_engine_wiring_preserves_no_mutation`).
...
The CognitiveOC software is licensed under the Apache License 2.0.

See the LICENSE file for details.

Note:
The corpus engineering framework tracks the original license of every
external dataset. Third-party datasets are **not relicensed** under
Apache 2.0 and remain subject to their respective licenses.
